...
export const JOB_PORTAL_URLS = [
  ...
  {
    name: "Roland Berger",
    url: "https://rolandberger-search-api.e-spirit.cloud/v1/prepared_search/JoinJobs/execute/?language=de&query=*&facet.filter.country=%22Deutschland%22&facet.filter.audiences=%22Consulting+-+Berufseinsteiger%22",
  }
];
...
export function parseJobsFromJson(data) {
  ...
  for (const obj of searchResults) {
    ...
    const title = job?.PositionTitle ?? job?.title?.join?.(" ") ?? job?.title ?? job?.jobTitle ?? "N/A";

    let location = "N/A";
    const posLoc = job?.PositionLocation ?? job?.positionLocation ?? job?.locations ?? job?.location ?? job?.cities;
    if (Array.isArray(posLoc)) {
      const cities = posLoc.map((j) => j?.CityName?.Name ?? j?.CityName ?? j?.City ?? j?.city ?? (typeof j === "string" ? j :
        "N/A"));
    ...
    const link = job?.PositionURI ?? job?.jobApplyURL ?? job?.url ?? job?.applyUrl ?? job?.url ?? "N/A";
